...
def get_patched_module(
    ...
    class ChatGLMAttention(nn.Module):
        ...
        split_tensor_along_last_dim: Callable[
            [torch.Tensor, int], Tuple[torch.Tensor, ...]
        ]
        ...
        def forward(
            ...
            matmul_result = torch.baddbmm(
                ...
                key_layer.transpose(0, 1).transpose(
                    1, 2
                ),  # [B * (HN + EN), HS, kv_length]
                beta=0.0,
                alpha=1.0,
            )
            ...
            value_layer = value_layer.view(
                kv_length, batch_size * num_heads, hidden_size
            )
    ...
    def patched_init(self: PreTrainedModel, config: PretrainedConfig, *args, **kwargs):
        ...
        with MonkeyPatch.context() as m:
            m.setattr(
                chatglm_module,
                "SelfAttention",
                get_patched_module(origin_attention, config_with_enhance),
            )
